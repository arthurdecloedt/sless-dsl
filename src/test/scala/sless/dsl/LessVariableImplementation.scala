...
object LessVariableImplementation {
  ...
  /**
    ...
    * @return
    */
  def doubledWidth(elementType: String, height: Int): dsl.Rule = tipe(elementType) (prop("height") := value(height.toString) , prop("width") := value((height*2).toString))
